Feature: Import and Export URL Groups

    Scenario: Importing and exporting a field group with an url field
        Given a WP install
        Then the exit code should be 0
        When I run the command "acf import --json_file='features/bootstrap/test_imports/url-group.json'"
        Then the exit code should be 0
        ...
        And the result should not be empty
        And the result string should start with "Success:"
        And the imported and exported "url-group.json" files should match
