...
Contributors: 	Hoppinger
Description: 	The goal of this project is to make life easier for developers who work together on Wordpress projects that use Advanced Custom Fields.

== 2. Description ==
This extension for Advanced Custom Fields makes it possible to manage your field_groups through the console of wp-cli. 
...
* added the acf command to wp-cli with subcommands:
	1. wp acf status: 	provides a dumped array of found field_groups in the current database of your Wordpress project.
	2. wp acf export: 	writes a field_group folder in httpdocs (or other root folder of your Wordpress installation).
	...
	3. wp acf import: 	Imports the xml field_groups that are found in the field_groups/{field_group_name}/data.xml
						You can now edit the field_groups in /wp-admin/edit.php?post_type=acf (use the awesome UI).
	4. wp acf clean:	Cleans up the database from all found acf post types and there coupled post_meta values
						Use this after you've edited the field_groups in the UI and used export to generate the new data.php files
	5. wp acf			Default test and help overview.
...
7. Start using the commands as explained in "5. Commands"


= 7. Feature requests, coming up soon =
